...
class Account:
    ...
    def take_loan(self, amount):
        ...
        if amount > 2 * self.balance:
            print("Loan amount exceeds double the available balance.Cannot take a loan.")
        elif self.num_loans_taken >= 2:  
            print("You have already taken the maximum number of loans (two).")
        else:
...
class Bank:
    ...
    def set_bankrupt(self, value):
        self.bankrupt = value
        if value:
            print("Bankrupt mode turned ON.")
        else:
            print("Bankrupt mode turned OFF.")
